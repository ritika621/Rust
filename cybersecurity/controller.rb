...
module Access
  ...
  def authenticate(user, passwd)
    ...
    query = %{
                ...
                result = db.get_first_value("SELECT Password FROM Users WHERE User = '#{user}'")
                               
                if result != passwd || no_user(user) != nil
...
module User
  ...
  #helper to check if there is no user returned from the query
 def no_user(user)
   user = Rack::Utils.escape_html(user)
   record = user
    query = %{
      SELECT User
        ...
        WHERE User = '#{user}' 
          ...
          @db.execute(query){ |user| record = nil}
...
true
	...
	def get_user(user)
	  ...
	  query = %{
		...
		WHERE User = '#{user}'
                ...
                query = @db.prepare "SELECT Avatar, Description FROM Users WHERE User = '#{user}'"
	...
	def update_prefs(user, session, description, token)
          user = Rack::Utils.escape_html(user)
          if(authorize(user, session) == false || authorize_token(user, token) == false)
            return false
          end
